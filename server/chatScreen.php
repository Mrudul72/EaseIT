...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($user_id != $sender_id) {
            $selectProPicSql = "SELECT profile_pic FROM tbl_user WHERE user_id = $sender_id";
            $selectProPicRes = mysqli_query($connect, $selectProPicSql);
            $selectProPicRow = mysqli_fetch_assoc($selectProPicRes);
            $profile_pic = $selectProPicRow['profile_pic'];
            if ($chat_text == NULL) {
                if ($chat_image != NULL && $chat_attachment == NULL) {
                    echo '
                    <div class="media w-50 mb-3"><img src="./assets/uploads/' . $profile_pic . '" alt="user" width="50" height="50" class="rounded-circle">
